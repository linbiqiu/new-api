...
type User struct {
	...
	FeishuDepartmentId         string `json:"feishu_department_id" gorm:"column:feishu_department_id;type:varchar(128);default:'';index"`
	FeishuDepartmentName       string `json:"feishu_department_name" gorm:"column:feishu_department_name;type:varchar(255);default:'';index"`
...
func GetAllUsers(pageInfo *common.PageInfo, accountType *int, sortOptions ...UserSortOptions) (users []*User, total int64, err error) {
	// Start transaction
	tx := DB.Begin()
	if tx.Error != nil {
	...
	err = order.Apply(dataTx).Limit(pageInfo.GetPageSize()).Offset(pageInfo.GetStartIdx()).Omit("password", "access_token").Find(&users).Error
	if err != nil {
		tx.Rollback()
		return nil, 0, err
	}
...
func SearchUsers(keyword string, group string, role *int, status *int, accountType *int, startIdx int, num int, sortOptions ...UserSortOptions) ([]*User, int64, error) {
	var users []*User
	var total int64
	var err error
