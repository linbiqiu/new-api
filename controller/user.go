...
func GetAllUsers(c *gin.Context) {
	...
	users, total, err := model.GetAllUsers(pageInfo, accountType, sortOptions)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func SearchUsers(c *gin.Context) {
	...
	pageInfo := common.GetPageQuery(c)
	sortOptions := model.NewUserSortOptions(c.Query("sort_by"), c.Query("sort_order"))
	users, total, err := model.SearchUsers(keyword, group, role, status, accountType, pageInfo.GetStartIdx(), pageInfo.GetPageSize(), sortOptions)
	if err != nil {
		common.ApiError(c, err)
		return
	}
