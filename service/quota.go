...
func PreWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.RealtimeUsage) error {
	...
	if userQuota < quota {
		return newWalletQuotaError(int64(userQuota), int64(quota))
	}

	if !token.UnlimitedQuota && token.RemainQuota < quota {
		return newAPITokenQuotaError(token.RemainQuota <= 0)
	}
...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
	...
	actualTokens := sumSettledTokens(usage.InputTokens, usage.OutputTokens)
	if err := SettleBilling(ctx, relayInfo, quota, actualTokens); err != nil {
...
func PostAudioConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent string) {
	...
	actualTokens := sumSettledTokens(usage.PromptTokens, usage.CompletionTokens)
	if err := SettleBilling(ctx, relayInfo, quota, actualTokens); err != nil {
		logger.LogError(ctx, "error settling billing: "+err.Error())
	}
